...
const FormDashboard = () => {
  ...
  const handleDeleteFolder = async () => {
    try {
      await axios.delete(`${BACKEND_URL}/api/folders/folder/${folderId}`, {
        headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
      });
      const updatedFolders = createFolders.filter(
  ...
  const handleDeleteForm = async () => {
    try {
      await axios.delete(`${BACKEND_URL}/api/folders/form/${formId}`, {
        headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
      });
        ...
        <div className={style.workspace_Navbar}>
          <div className={style.navbar}>
            <select name="dropdown" id="dropdown" onChange={handleSetting}>
              ...
              <option value="setting" className={style.setting}>
                Settings
              </option>
            ...
            <div className={style.Create_Folder}>
              <div className={style.Folder}>
                ...
                {createFolders.map((folder, index) => (
                  <div
                    key={index}
                    className={style.New_folder}
                    onClick={() => {
                      handlegetForms(folder);
                    }}
                  >
                    {folder.name}
                    <i
              ...
              <div className={style.Folder_Form}>
                ...
                {newForm.map((form, index) => (
                  <div
                    key={index}
                    className={style.New_folderForm}
